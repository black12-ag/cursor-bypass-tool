...
Github: https://example.com/munir-temp-link{Fore.RED}
Press 8 to change language | 按下 8 键切换语言{Style.RESET_ALL}"""

# center display LOGO and DESCRIPTION
BYPASS_CURSUR_LOGO = center_multiline_text(LOGO_TEXT, handle_chinese=False)
BYPASS_CURSUR_DESCRIPTION = center_multiline_text(DESCRIPTION_TEXT, handle_chinese=False)
BYPASS_CURSUR_OTHER_INFO = center_multiline_text(OTHER_INFO_TEXT, handle_chinese=True)

def print_logo():
    print(BYPASS_CURSUR_LOGO)
    print(BYPASS_CURSUR_DESCRIPTION)
    print(BYPASS_CURSUR_OTHER_INFO)
