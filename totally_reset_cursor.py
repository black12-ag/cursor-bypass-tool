    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(mode="w", encoding="utf-8", errors="ignore", delete=False) as tmp_file:
            ...
            if sys.platform == "win32":
                ...
                CButton_new_pattern = r'$(k,E(Ks,{title:"MUNIR KABIR",size:"small",get codicon(){return F.rocket},get onClick(){return function(){window.open("https://example.com/munir-temp-link","_blank")}}}),null)'
                ...
                CButton_new_pattern = r'M(x,I(as,{title:"MUNIR KABIR",size:"small",get codicon(){return $.rocket},get onClick(){return function(){window.open("https://example.com/munir-temp-link","_blank")}}}),null)'
...
class MachineIDResetter:
    ...
    def generate_new_ids(self):
        """Generate new machine IDs and UUIDs"""
        new_machine_id = hashlib.sha256(str(uuid.uuid4()).encode()).hexdigest()
        new_guid = str(uuid.uuid4())
        new_platform_uuid = str(uuid.uuid4())
        return {
            "machine_id": new_machine_id,
            "windows_guid": new_guid,
            "macos_platform_uuid": new_platform_uuid,
        }
    ...
    def _update_macos_platform_uuid(self, new_ids):
        ...
        try:
            ...
            if os.path.exists(uuid_file):
                ...
                cmd = f'sudo plutil -replace "UUID" -string "{new_ids["macos_platform_uuid"]}" "{uuid_file}"'
...
def run(translator=None):
    """Run cursor totally reset"""
    print(f"\n{Fore.CYAN}{'='*50}{Style.RESET_ALL}")
    print(f"{Fore.CYAN}{EMOJI['RESET']} {translator.get('reset.title') if translator else 'Cursor Reset Tool'}{Style.RESET_ALL}")
...
{Style.RESET_ALL}""")
    ...
    try:
        ...
        print(f"\n{Fore.CYAN}{'='*50}{Style.RESET_ALL}")
        input(f"{EMOJI['INFO']} {translator.get('reset.press_enter') if translator else 'Press Enter to continue...'}...")
        return True
    except Exception as e:
        print(f"{Fore.RED}{EMOJI['ERROR']} {translator.get('reset.process_error', error=str(e)) if translator else f'Error during reset process: {str(e)}'}{Style.RESET_ALL}")
        return False
