...
def display_account_info(translator=None):
    ...
    token = get_token()
    if not token:
        print(f"{Fore.YELLOW}{EMOJI['INFO']} {translator.get('account_info.token_not_found') if translator else 'No active Cursor account found. Please register or login first.'}{Style.RESET_ALL}")
        return
